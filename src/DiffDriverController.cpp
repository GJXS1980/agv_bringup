...
DiffDriverController::DiffDriverController(double max_speed_,std::string cmd_topic_,StatusPublisher* hg_status_,CallbackAsyncSerial* cmd_serial_car_,CallbackAsyncSerial* cmd_serial_imu_,double r_min)
{
    MoveFlag=true;
    max_wheelspeed=max_speed_;
    cmd_topic=cmd_topic_;
    hg_status=hg_status_;
...
void DiffDriverController::Refresh()
{
  ...
  if(t_diff.toSec()<10.0 && (hg_status->car_status.hbz_status & 0x01)==1 && !shutdown_flag_)
  {
    if(t_diff.toSec()>3.0 || hg_status->get_status()<=0)
    {
...
void DiffDriverController::send_speed()
{
  ...
  separation = hg_status->get_wheel_separation();
  radius = hg_status->get_wheel_radius();
  ...
  if(NULL!=cmd_serial_car)
  {
    ...
    hgserial_server::CRC16CheckSum((unsigned char *)speed_cmd, 10, crc_hl);
...
void DiffDriverController::UpdateSpeed()
{
  ...
  geometry_msgs::Twist car_twist_now =  hg_status->get_CarTwist();
  ...
  float bar_distance = hg_status->get_ultrasonic_min_distance();
...
void DiffDriverController::filterGoal()
{
  ...
  if(DetectFlag_)
  {
    ...
    hg_status->get_canmove_flag(forward_flag,rot_flag);
  ...
  if(galileoStatus_.nav_status ==1 )
  {
    if(galileoStatus_.visual_status != 0)
    {
      ...
      // {
        ...
        {
          //判断c3开关是否按下
          if((hg_status->car_status.hbz_status & 0x04)==1)
          {
          ...
          else
          {
            ...
            if(back_touch_falg_ && t_diff.toSec()>=0.1)
            {
              ...
              // currentCmds.header.frame_id = "hg_serial_server";
