...
StatusPublisher::StatusPublisher()
{
   ...
   mStatusFlagPub = mNH.advertise<std_msgs::Int32>("StatusFlag",1,true);
   mDriverFlagPub = mNH.advertise<std_msgs::Int32>("DriverFlag",1,true);
   ...
   pub_barpoint_cloud_ = mNH.advertise<PointCloud>("barpoints", 1, true);
   pub_clearpoint_cloud_ = mNH.advertise<PointCloud>("clearpoints", 1, true);

   mSonar1Pub = mNH.advertise<sensor_msgs::Range>("Sonar1", 1, true);
   mSonar2Pub = mNH.advertise<sensor_msgs::Range>("Sonar2", 1, true);
   mSonar3Pub = mNH.advertise<sensor_msgs::Range>("Sonar3", 1, true);
   mSonar4Pub = mNH.advertise<sensor_msgs::Range>("Sonar4", 1, true);

   mIMUPub = mNH.advertise<sensor_msgs::Imu>("IMU", 1, true);
...
void StatusPublisher::Update_car(const char data[], unsigned int len)
{
    ...
    for(i=0;i<len;i++)
    {
        ...
        hgserial_server::CRC16CheckSum(cmd_string_buf_copy, 10, crc_hl);
