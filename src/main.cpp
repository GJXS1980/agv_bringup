...
{
    // cout<<"welcome to HG AGV serial server,please feel free at home!"<<endl;
    ros::init(argc, argv, "hgserial_server");
    ros::start();
    ...
    hgserial_server::StatusPublisher hg_status(separation,radius,power_scale);
    hg_status.setBarParams(rot_dist,tran_dist);
    //获取小车控制参数
    ...
    ros::Publisher audio_pub = mNH.advertise<std_msgs::String>("tts/text", 1, true);

    try {
      //  下位机电机连接
      CallbackAsyncSerial serial_car(port_car,baud_car);
      serial_car.setCallback(boost::bind(&hgserial_server::StatusPublisher::Update_car,&hg_status,_1,_2));

      //  传感器板IMU连接
      CallbackAsyncSerial serial_imu(port_imu,baud_imu);
      serial_imu.setCallback(boost::bind(&hgserial_server::StatusPublisher::Update_imu,&hg_status,_1,_2));

      //  底盘运动学求解
      hgserial_server::DiffDriverController hg_diffdriver(max_speed,cmd_topic,&hg_status,&serial_car,&serial_imu,r_min);
      hg_diffdriver.setBarParams(angle_limit,tran_dist,x_limit,y_limit);
      boost::thread cmd2serialThread(& hgserial_server::DiffDriverController::run,&hg_diffdriver);
      ...
      const char driver_clear_cmd[8] = {(char)0x01,(char)0x06,(char)0x46,(char)0x02,(char)0x00,(char)0x00,(char)0x3d,(char)0x42}; //  清除模式
      const char driver_reset_cmd[8] = {(char)0x01,(char)0x06,(char)0x46,(char)0x03,(char)0x00,(char)0x01,(char)0xad,(char)0x42}; //  复位模式
      const char driver_read_error_cmd[8] = {(char)0x01,(char)0x43,(char)0x50,(char)0x12,(char)0x51,(char)0x12,(char)0x48,(char)0x9d}; //模式
      const char driver_read_odom_cmd[8] = {(char)0x01,(char)0x43,(char)0x50,(char)0x04,(char)0x51,(char)0x04,(char)0x28,(char)0x97}; //  里程计模式
      const char driver_read_enable_cmd[8] = {(char)0x01,(char)0x43,(char)0x21,(char)0x00,(char)0x31,(char)0x00,(char)0x5b,(char)0xa9}; //  使能模式
      ...
      while (ros::ok())
      {
        ...
        {
          //每隔2秒下发心跳包
          hg_diffdriver.sendHeartbag();
        }

        serial_car.write(driver_read_odom_cmd,8);
        usleep(8000);//延时8MS，等待数据上传
        hg_status.Refresh();//定时发布状态
        if(i%2==0)
        {
          hg_diffdriver.Refresh();//更新驱动速度 25hz
        }
        //如果驱动器有报警，需要清除报警，清除报警3次后都恢复不了，就重启驱动器
        if(hg_status.car_status.driver_error != 0 && i%10==0)
        {
          if(clear_try<3)
          {
            //清除错误
            ROS_ERROR("clear motor driver error! %d %d ",hg_status.car_status.driver_error1, hg_status.car_status.driver_error2);
          ...
          {
            //复位，需要重置一些状态
            ROS_ERROR("reset motor driver ! %d %d ",hg_status.car_status.driver_error1, hg_status.car_status.driver_error2);
            serial_car.write(driver_reset_cmd,8);
            hg_status.ResetDriver();
            hg_diffdriver.ResetDriver();
            clear_try = 0;
          }
        }
        else
        ...
        //更新按钮
        if(hg_diffdriver.dealBackSwitch())
        {
          //告诉用户回去了
